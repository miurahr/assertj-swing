...
public class ShouldBeInSameSecond extends BasicErrorMessageFactory {
  ...
  /**
   * Creates a new </code>{@link ShouldBeInSameSecond}</code>.
   * @param actual the actual value in the failed assertion.
   * @param other the value used in the failed assertion to compare the actual value to.
   * @return the created {@code ErrorMessageFactory}.
   */
  public static ErrorMessageFactory shouldBeInSameSecond(Date actual, Date other) {
    return new ShouldBeInSameSecond(actual, other);
  }

  private ShouldBeInSameSecond(Date actual, Date other) {
    super("expected <%s> to be on same year, month, day, hour, minute and second as <%s>", actual, other);
  }
}
