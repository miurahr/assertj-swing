...
public class JTableDriver_requireCellValueAsPattern_Test extends JTableDriver_TestCase {
  ...
  public void should_Fail_If_Cell_Value_Does_Not_Match_Pattern() {
    Throwable t = Assert.assertThrows(AssertionError.class, () -> driver.requireCellValue(table, row(0).column(0),
            Pattern.compile("0-1")));
    assertThat(t.getMessage()).contains("value [row=0, column=0]")
            .contains("0-0").contains("to match pattern:").contains("\"0-1\"");
  }
}
